...
| A three-training-seed LOGO reconstruction audit is attached as aggregate-only evidence. | `experiments/logo_capacity_reconstruction/` | The observed noisy capacity is an input. Across 18 seed-folds, PINN RMSE is unstable and does not beat LSTM; shared running-min post-processing yields 0% upward-step violations for both. |
...
|------|----------------|-----------|
| PINN reconstruction error remains stable across training seeds. | The three-seed audit finds strong PINN instability, especially at seed 123. | Diagnose optimization and data-pipeline causes under a preregistered follow-up; do not infer forecasting performance from this reconstruction protocol. |
